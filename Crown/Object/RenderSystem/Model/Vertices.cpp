...
void Crown::RenderObject::Vertices::CreateBuffer(ID3D12Device* device, std::vector<VerticesData>& vertices, std::vector<unsigned int>& indexBuffer)
{
	...
	D3D12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(ResourceUploader::GetInstance()->Get255AlignmentSize<VerticesData>(vertices.size()));
	...
	ResourceUploader::GetInstance()->UploadData<VerticesData>(m_vertexBuffer.Get(), ResourceUploader::GetInstance()->Get255AlignmentSize<VerticesData>(vertices.size()), [&](VerticesData* map)
		{
	...
	m_vertexBufferView.SizeInBytes = ResourceUploader::GetInstance()->Get255AlignmentSize<VerticesData>(vertices.size());
	m_vertexBufferView.StrideInBytes = sizeof(VerticesData);


	heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
	resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(ResourceUploader::GetInstance()->Get255AlignmentSize<unsigned int>(indexBuffer.size()));
	...
	ResourceUploader::GetInstance()->UploadData<unsigned int>(m_indexBuffer.Get(), ResourceUploader::GetInstance()->Get255AlignmentSize<unsigned int>(indexBuffer.size()), [&](unsigned int* map)
		{
	...
	m_indexBufferView.SizeInBytes = ResourceUploader::GetInstance()->Get255AlignmentSize<unsigned int>(indexBuffer.size());
